...
# Загрузка тестовой модели и энкодеров
try:
    model_test = joblib.load(f'{MODEL_DIR}/real_model.pkl')
except Exception as e:
    print(f"[ERROR] Не удалось загрузить model_test: {e}")
    model_test = None
...
try:
    ...
    print(f"[ERROR] Не удалось загрузить энкодеры: {e}")
    le_region = le_device = None

model_new = None  # будет создан при обучении
...
def upload():
    try:
        ...
        # Обработка энкодеров (внутри одного try)
        try:
        ...
        X = df[features]
        model = model_test if model_type == 'TEST' else model_new

        if model is None:
            return jsonify({'error': 'Модель не загружена или не обучена.'}), 400
